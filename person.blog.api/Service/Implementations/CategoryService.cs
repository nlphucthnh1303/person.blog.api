...
namespace PersonBlogApi.Services.Implementations
{
    public class CategoryService : BaseService, ICategoryService
    {
        ...
        public async Task<List<CategoryGetAll_Res>> CategoryGetAll()
        {
            using (var connection = GetConnection())
            {
                return (await connection.QueryAsync<CategoryGetAll_Res>(
        ...
        public async Task<CategoryGetById_Res?> CategoryGetById(int categoryId)
        {
            using (var connection = GetConnection())
            {
                var parameters = new { p_CategoryId = categoryId };
                return await connection.QueryFirstOrDefaultAsync<CategoryGetById_Res>(
        ...
        public async Task<CategoryGetBySlug_Res?> CategoryGetBySlug(string slug)
        {
            using (var connection = GetConnection())
            {
                var parameters = new { p_Slug = slug };
                return await connection.QueryFirstOrDefaultAsync<CategoryGetBySlug_Res>(
        ...
        public async Task<bool> CategoryUpdate(CategoryUpdate category)
        {
            using (var connection = GetConnection())
            {
                // Create dynamic parameters or extend CategoryUpdateDto to include CategoryId
                var parameters = new DynamicParameters();
                parameters.Add("p_CategoryId", category.CategoryId);
                parameters.Add("p_Name", category.Name);
                parameters.Add("p_Slug", category.Slug);
                parameters.Add("p_ParentCategoryId", category.ParentCategoryId);
